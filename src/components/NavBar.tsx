import { useRef, useEffect, useState } from "react"; // Importamos useEffect y useState además de useRef
import { useDispatch, useSelector } from "react-redux";
import { Link, useLocation } from "react-router-dom";
import NavButton from "./NavButton";
import { captureText } from "../actions/products";

export default function NavBar() {
  const location = useLocation();
  const pathname = location.pathname;
  const text = useRef<HTMLInputElement>(null); // Especificamos el tipo HTMLInputElement para text
  const dispatch = useDispatch();
  const textStore = useSelector((store: any) => store.products.text);
  
  // Estado para mantener la cuenta de productos en el carrito
  const [cartCount, setCartCount] = useState(0);

  // Función para capturar el texto del input
  const setText = () => {
    dispatch(
      captureText({
        text: text.current?.value,
      })
    );
  };

  // useEffect para leer la cantidad de productos del local storage cuando el componente se monta
  useEffect(() => {
    const cartItems = JSON.parse(localStorage.getItem("cart") || "[]");
    setCartCount(cartItems.length); // Establecemos el estado con la cantidad de productos
  }, []); // El array vacío significa que esto se ejecuta solo una vez al montar el componente

  return (
    <header className="w-full min-h-[150px] bg-[#04585d] p-[20px] flex flex-col items-center">
      <div
        className="w-full flex flex-col justify-between items-center grow gap-[20px] pb-[20px]
                      md:flex-row xl:w-[1080px]"
      >
        <Link to="/" className="w-1/3 flex items-center grow justify-start">
          <img
            className="h-[50px] items-center"
            src="/tiendamia-logo.svg"
            width="auto"
            alt="Logo store"
          />
        </Link>

        <form className="w-10/12 md:w-1/3 flex items-center grow">
          <input
            className="h-[60px] rounded-[15px] w-full p-[10px] mx-[20px] text-[14px] text-center outline-none"
            type="text"
            placeholder="Search"
            id="search"
            ref={text}
            onChange={setText} // Cambiado a onChange para manejar correctamente el evento
          />
        </form>

        <ul className="w-1/3 flex items-center grow justify-center md:justify-end list-none">
          <li id="facebook" className="w-[50px]">
            <a href="https://facebook.com">
              <img
                className="w-[30px] h-[30px]"
                src="data:image/png;base64,"
              />
            </a>
          </li>
          <li id="instagram" className="w-[50px]">
            <a href="https://instagram.com">
              <img
                className="w-[30px] h-[30px]"
                src="data:image/png;base64,"
              />
            </a>
          </li>
          <li id="cart" className="w-[50px]">
            <Link className="flex text-white" to="/cart">
              <img
                className="w-[30px] h-[30px]"
                src="data:image/png;base64,"
              />
              <p>{cartCount}</p> {/* Muestra la cantidad de productos en el carrito */}
            </Link>
          </li>
        </ul>
      </div>

      <nav
        id="navbar"
        className="w-full p-[15px 20px] bg-[#04585d] flex flex-wrap justify-center md:justify-between list-none items-center text-[14px] xl:w-[1080px]"
      >
        <NavButton title="Ofertas" link="/" />
        <NavButton title="Cómo comprar" link="/" />
        <NavButton title="Costos y tarifas" link="/" />
        <NavButton title="Mis pedidos" link="/" />
        <NavButton title="Garantía" link="/" />
      </nav>
    </header>
  );
}
